...
// TODO: Destructo has found a map with coordinates marked on it. Use object destructuring 
...
const {upper, lower, middle = upper} = doorCode;
...
// Make sure the `middle` defaults to one of the values from the `remaining` variable above if not provided. 
// Print the door code sequence.
...
const[stoneOne, , , , stoneSix] = stones;
console.log(stoneOne, stoneSix); 
// TODO: Crossing the River of Reflections requires skipping certain stones. 
// Use array destructuring to extract only the first and the sixth stones. Print the extracted stones.
...
// Help him by writing a function `revealPath` that decodes and prints the 
...
const ancientDirections = {
	direction: "North",
	distance: 100
};
revealPath(ancientDirections);

/* Task 9: The Scroll of Defaults */
// TODO: Destructo finds an ancient scroll with a potion recipe, 
// but some ingredients are missing. 
// Write a function `mixPotion` that uses defaults "Water" and "Fireflower" 
// for `ingredient1` and `ingredient2` if they are not specified and print those mixings. 
// The function takes an object with these optional properties.

const mixPotion = ({ ingredient1 = "Water", ingredient2 = "Fireflower"} = {}) => {
	console.log('Mixing a potion with Water and Fireflower. ');
};
mixPotion({});

/* Task 10: The Array Spell */
// TODO: At the gates of an ancient library, Destructo must cast a spell 
// with the first two ingredients from a list given to him by a wise owl. 
// 6Create a function `castSpell` that uses array destructuring to access these 
// ingredients from an array and print the spell casting.
function castSpell(ingredients) {
	const [ingredient1 = "Unicorn Horn", ingredient2 = "Drangons Breath"] = ingredients;
	console.log('casting a spell with ingrediet1 and ingredient2.');
};
const Spellingredients = ["Unicorn Horn", "Dragons breath"];
